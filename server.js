...
app.get('/api/responses', async (req, res) => {
    try {
        const responses = await getCollection().find({}).toArray();
        res.json(responses);
    } catch (error) {
...
app.post('/api/responses', async (req, res) => {
    try {
        ...
        const result = await getCollection().insertOne(response);
        res.status(201).json({
...
app.get('/api/stats', async (req, res) => {
    try {
        const allResponses = await getCollection().find({}).toArray();
